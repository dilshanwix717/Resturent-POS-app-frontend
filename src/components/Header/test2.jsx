...
import { fetchSellingTypes, logoutUser, checkDailyBalance } from '../../api/apiService';
  ...
  useEffect(() => {
    const checkBalance = async () => {
      ...
      try {
        ...
        console.warn("Daily balance entry not found. Logging out.");
        toast.error("Daily balance entry not found. You will be logged out.");
        logoutUser();
        ...
        <div className='flex items-center gap-5'>
          <nav className="flex-grow">
            <ul className="flex justify-center space-x-4 text-xs xl:text-sm">
              {isPosPage && sellingTypes.map((type) => (
                <li key={type._id}>
                  <a
                    className={`cursor-pointer ${activeOption === type.sellingTypeId ? 'text-secondary hover:text-secondary' : 'hover:text-secondary'}`}
                    onClick={() => handleOptionClick(type.sellingTypeId)}
                  >
                    {type.sellingType}
                  </a>
                </li>
