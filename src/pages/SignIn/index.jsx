...
const SignIn = ({ setAuth, setDayStarted }) => {
  ...
  useEffect(() => {
    const loadBranches = async () => {
      try {
        ...
        const enabledBranches = branchData.filter(
          (branch) => branch.toggle === "enable"
        );
    ...
    const loadCompanies = async () => {
      try {
        ...
        const enabledCompanies = companyData.filter(
          (company) => company.toggle === "enable"
        );
  ...
  const handleLogin = async (e) => {
    ...
    const selectedBranchDetails = branches.find(
      (branch) => branch.shopId === selectedBranch
    );
    ...
    try {
      ...
      if (response.data.userId) {
        const { sessionToken, userId, username, companyId, shopId, email } =
          response.data;
        setAuth(true);
        ...
        try {
          ...
          const dayStartResponse = await checkDailyBalance(
            companyId,
            shopId,
            today
          );
          ...
          if (dayStartError.response && dayStartError.response.status === 404) {
            console.warn(
              "No daily balance entry found for the specified date."
            );
            navigate("/dashboard");
      ...
      <div className="bg-primary w-full h-full rounded-xl relative">
        ...
        <div className="absolute flex flex-col gap-3 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 min-w-96 bg-white p-10 rounded-xl z-10">
          <div className="flex justify-center">
            <img
              src="/assets/logo.png"
              alt="Savour-street-logo"
              className="w-40"
            />
          </div>
          <div className="w-full flex flex-col gap-1">
            <label htmlFor="username" className="text-xs xl:text-sm">
              User Name
            </label>
            <input
          ...
          <div className="w-full flex flex-col gap-1">
            <label htmlFor="password" className="text-xs xl:text-sm">
              Password
            </label>
          ...
          <div className="w-full flex flex-col gap-1">
            <label htmlFor="company" className="text-xs xl:text-sm">
              Company
            </label>
            <select
          ...
          <div className="w-full flex flex-col gap-1">
            <label htmlFor="branch" className="text-xs xl:text-sm">
              Branch
            </label>
            <select
              className="block w-full text-xs xl:text-sm bg-gray-100 text-gray-700 border border-gray-100 rounded py-2 px-4 leading-tight focus:outline-secondary focus:bg-gray-50"
            ...
            >
              ...
              {filteredBranches.map((branch) => (
                <option key={branch.shopId} value={branch.shopId}>
                  {branch.shopName}
                </option>
              ))}
            </select>
