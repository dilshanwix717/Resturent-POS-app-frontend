...
const POS = ({ selectedOption }) => {
        ...
        return prevCart.map((item) =>
          item.name === product.name && item.size === product.size
            ? {
                ...
                finalPrice: (item.quantity + 1) * item.price,
              }
            : item
        );
      } else {
  ...
  const updateCartItem = (_id, action, newSize) => {
    setCart((prevCart) =>
      prevCart.map((item) =>
        item._id === _id
          ? {
              ...
              finalPrice: Math.max(action === "increment" ? item.quantity + 1 : item.quantity - 1, 1) * item.price,
            }
          : item
      )
    );
